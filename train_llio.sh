#!/usr/bin/env bash
# =============================================================================
# Train the LLIO-Net (ResMLP512) backbone under THIS repo's protocol and
# evaluate it on seen/unseen subjects (backbone only, no RF).
#
...
# feature_dim 512, patch 25 -> 50 to hold 0.25 s/patch at our 200 Hz, mean-pool
# + linear + GELU head, dropout 0.2), trained with OUR protocol on OUR RoNIN
...
#
# Runtime: 7.27M params, MLP-only ops. Budget a few hours for 100 epochs on one
# A6000. Run it detached, e.g.:
#   GPU=0 nohup bash train_llio.sh > train_llio.out 2>&1 &
#
...
echo "python: ${PYTHON}"
echo "training root: ${TRAIN_ROOT} | GPU ${GPU} | epochs ${EPOCHS} | out: ${OUT_DIR}"

# ---- [1] train ----------------------------------------------------------------
echo "=== [1/3] training LLIO-Net (ResMLP512) for ${EPOCHS} epochs on GPU ${GPU}"
...
echo
echo "=== RESULTS: LLIO-Net ResMLP512, ${EPOCHS}-epoch MuSGD run (best epoch ${best_epoch})"
for split in seen unseen; do
    printf "  %-7s " "${split}"
...
echo "    LLIO-Net ResMLP512 (this run)                                            7.27M params"
