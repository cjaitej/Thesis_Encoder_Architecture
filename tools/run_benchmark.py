#!/usr/bin/env python3
"""
Run a command multiple times and extract per-run timing metrics.
Writes CSV with columns: run_idx,sample_time_ms,seq_time_ms,returncode

Default command is the ResNet test command run in the repository `source` directory.
"""
import argparse
import shlex
import subprocess
...
# Default commands (run from `source` directory)
COMMANDS = {
    'resnet': (
        "python ronin_resnet_baseline_plain.py --mode test --dataset ronin "
        "--model_path ../ronin_resnet/checkpoint_gsn_latest.pt "
        "--root_dir ../data/seen_subjects_test_set "
        "--test_path ../data/seen_subjects_test_set/a001_2 "
        "--out_dir ../output/timing_resnet_a001_2"
    ),
    'yolo': (
        "python ronin_yolo26_baseline_plain.py --mode test --dataset ronin "
        "--model_path ../output/yolo26_adam_attn_t2_r3/checkpoints/checkpoint_35.pt "
        "--root_dir ../data/seen_subjects_test_set "
        "--test_path ../data/seen_subjects_test_set/a001_2 "
        "--out_dir ../output/timing_yolo26_a001_2 --model_dropout 0.2"
    ),
    'yolo_rf': (
        "python ronin_yolo26_baseline_plain.py --mode test --dataset ronin "
        "--model_path ../output/yolo26_adam_attn_t2_r3/checkpoints/checkpoint_35.pt "
        "--root_dir ../data/seen_subjects_test_set "
        "--test_path ../data/seen_subjects_test_set/a001_2 "
        "--out_dir ../output/timing_yolo26_rf_a001_2 --use_attention --model_dropout 0.2 "
        "--use_rf_postprocess --rf_model_path ../output/rf_yolo26/rf_corrector.joblib "
        "--rf_alpha 1.0 --rf_clip 0.75 --rf_hist_window 50"
    ),
}
...
RE_YOLO_RF = re.compile(r"neural=([0-9]+\.?[0-9]*)ms,\s*rf=([0-9]+\.?[0-9]*)ms,\s*total=([0-9]+\.?[0-9]*)ms", re.IGNORECASE)
...
def extract_times(stdout_text):
    ...
    if m:
        try:
            ...
            pass
    # Try YOLO RF combined pattern (prefer total/sample if present)
    m = RE_YOLO_RF.search(stdout_text)
    if m:
        try:
            total_ms = float(m.group(3))
            # many logs include time_per_sample later; if not, we can return None for sample
...
def run_repeat(cmd_list, cwd, runs, delay):
    results = []
...
def main():
    ...
    parser.add_argument('--which', type=str, default='all', help="Which predefined command to run: 'all', 'resnet', 'yolo', 'yolo_rf'")
    ...
    for name, cmd in to_run:
        ...
        rows = run_repeat(cmd_list, cwd=str(cwd), runs=args.runs, delay=args.delay)
