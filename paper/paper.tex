...
position. This design rests on the premise that a compact temporal
backbone can retain competitive accuracy under a tight parameter
budget, and that the resulting residual errors are structured enough,
locally and in part across architectures, for a causal corrector to
exploit. Together with the budget formalized in
Section~\ref{sec:sysmodel}, this frames the paper's central question:
whether correction moves the accuracy--latency--footprint Pareto
frontier for edge inertial odometry, rather than simply optimizing
accuracy in isolation. The main contributions are:
\begin{itemize}
  ...
  \item We profile the latency, power and energy of multiple lightweight backbones on a Raspberry Pi and test whether the same residual corrector, fit once on ResidualNav-Backbone, transfers without refitting, finding transfer for most but not all backbones.
\end{itemize}
...
\cite{airimu2023}). These methods improved accuracy, but often at the
cost of larger models and higher inference latency.
...
trajectory smoothing improve consistency \cite{ronin2019} at the cost
of added complexity. Residual correction is a simpler alternative
that learns structured errors left by a primary predictor, the idea
behind our lightweight Random Forest postprocessor for short-horizon
velocity error before trajectory integration
\cite{solin2018,breiman2001,rf_imu_2023}.


\subsection{Lightweight Neural Architectures for Edge Deployment}

Running deep neural networks on edge devices requires post-hoc
compression (knowledge distillation, pruning, quantization, all
sensitive to calibration data and teacher-model quality) or
efficient-by-design architectures. Architecturally efficient designs such as MobileNetV2
...
{\fboxrule=1pt\fcolorbox{blue!60}{blue!15}{\parbox[c][1.3cm][c]{0.92\linewidth}{\centering\scriptsize\bfseries ResidualNav-Backbone}}}\\[2pt]
...
ResidualNav-Backbone to produce raw planar velocity predictions
...
\subsection{Backbone Architecture: ResidualNav-Backbone}
...
ResidualNav-Backbone: the spatial detection operations are replaced by 1D
...
the full trajectory. ResidualNav-Backbone achieves this with only 598,530
...
\begin{table}[htbp]
\caption{Layer stack of ResidualNav-Backbone for a $6\times200$ input.}
\label{tab:yolo_arch}
\centering
\scriptsize
\begin{tabular}{|l|l|r|}
...
corrector, predicting and removing local residual patterns in
$\hat{\mathbf{v}}_t$ to reduce step-wise bias and variance, and with
them the accumulated drift reflected in trajectory error over long
paths.
...
Random Forests \cite{breiman2001} are chosen for four reasons: (i) they
handle nonlinear feature--residual relationships without careful
architecture search; (ii) they are robust on structured tabular data
with mixed feature scales; (iii) they offer fast deterministic
inference once trained; and (iv) they natively support multi-output
regression for joint $(x, y)$ velocity residual prediction.

\subsubsection{End-to-End Pipeline}

The full inference pipeline proceeds as:
\begin{enumerate}
  \item Run ResidualNav-Backbone to obtain $\hat{\mathbf{v}}_t$
        for all timesteps.
...
by the run's elapsed wall-clock time to give energy: $\mathrm{Energy/run} =
\mathrm{board\ power} \times \mathrm{total\ wall\text{-}clock\ time\ to\ process\ one\ complete\ sequence}$.
Latency is reported per window (total wall-clock time divided by the
sequence's window count) to compare against the 50~ms budget, so it is a
much smaller quantity than the per-run energy and power, which are
inherently per-sequence: multiplying the reported latency by the reported
power does not reproduce the reported energy, since a run integrates
power over several thousand windows, not one. Latency, power and energy are
...
Alongside the RoNIN-ResNet baseline and proposed ResidualNav-Backbone
pipeline, we evaluate five competing lightweight 1D backbones to test
...
\begin{enumerate}
  ...
  \item \textbf{ResidualNav-Backbone (No RF):} The 0.60M-parameter backbone
        of Section~\ref{sec:method}, trained identically and evaluated
        without postprocessing.
  \item \textbf{ResidualNav-Backbone + RF (Proposed):} ResidualNav-Backbone
  ...
  \item \textbf{Competing lightweight backbones:} TinyCNN-1D,
        ...
        Random Forest fitted to ResidualNav-Backbone's residuals
        (Section~\ref{sec:method}) is applied unchanged, so their
        corrected results measure how well ResidualNav transfers
        across backbones.
  \item \textbf{LLIO-Net (ResMLP128):} our reimplementation of
        LLIO's~\cite{llio2022} lightweight variant, trained and RF-transferred
        the same way, since the official repository ships only the
        architecture.
\end{enumerate}
...
ResidualNav-Backbone uses Adam, no weight decay and head dropout 0.5 for up
...
100 epochs (RoNIN-ResNet, ShuffleNetV2-1D, MobileNetV2-1D, LLIO-Net)
or 200 (TinyCNN-1D, LightTCN-1D); every model plateaued well before
its budget. Although RoNIN trained its ResNet with Adam, our MuSGD
...
validation set. For ResidualNav-Backbone, the residual correction uses
...
The next three subsections evaluate whether the compact backbone retains competitive accuracy under its parameter budget.

\subsection{Parameter Efficiency}

Table~\ref{tab:model_footprint} separates neural backbone footprint from residual-correction complexity. ResidualNav-Backbone contains 0.60M trainable parameters, an 87.1\% reduction relative to RoNIN-ResNet. ResidualNav employs a 400-tree Random Forest over 17 input features; as a non-neural component, its storage cost is reported separately and is not included in the neural parameter count or FP32 weight size.
...
\hline
ResidualNav-Backbone & 598,530 & 2.28 MiB \\
\textbf{ResidualNav-Backbone + RF} & \textbf{598,530} &
\textbf{2.28 MiB} \\
...
Table~\ref{tab:pi_tflite_latency} reports the per-window latency, average board power and measured energy per run of the TFLite backbones on the Raspberry Pi 3 B+, where a \emph{run} is one full seen-subject sequence (Section~\ref{sec:exp}), not one inference window: for ResidualNav-Backbone, the 50 profiled runs average 187.9~s of wall-clock time and 7,415 windows per sequence, so $\mathrm{Energy/run} = \mathrm{power} \times \mathrm{total\ sequence\ time} \approx 3.55~\mathrm{W} \times 187.9~\mathrm{s} \approx 671.78~\mathrm{J}$; multiplying 25.31~ms by 3.55~W instead gives only $\approx$0.09~J, a single window's energy, not a run's. ResidualNav-Backbone runs in 25.31~ms per window, within the 50~ms update budget with 24.7~ms of headroom, and is $3.5\times$ faster than RoNIN-ResNet (89.42~ms) and $3.2\times$ faster than MobileNetV2-1D (80.08~ms), both of which exceed the budget. Average board power varies from 3.25~W (LightTCN-1D) to 4.34~W (TinyCNN-1D), only a $1.34\times$ spread against an $8.1\times$ spread in latency, so energy is governed mainly by latency rather than power: ResidualNav-Backbone consumes 671.78~J per run against 2269.86~J for RoNIN-ResNet, a $3.4\times$ reduction. This latency dominance produces a \emph{race-to-sleep} effect~\cite{albers2010energy}: TinyCNN-1D draws the highest average power of any model (4.34~W) yet has the lowest energy per run (359.75~J), because its 11.09~ms latency lets the board return to a lower-power idle state sooner than the slower alternatives. TinyCNN-1D and ShuffleNetV2-1D also meet the budget, while LightTCN-1D narrowly misses it at 50.11~ms; after correction, TinyCNN-1D is less accurate on both metrics and ShuffleNetV2-1D has a higher ATE (Table~\ref{tab:seen}). Table~\ref{tab:pi_tflite_latency} also reports ResidualNav-Backbone+RF: latency rises only 0.30~ms (25.31 to 25.61~ms, still 24.4~ms under budget) and power/energy increase modestly (3.55 to 3.77~W; 671.78 to 744.81~J) with overlapping standard deviations, so correction does not meaningfully change the deployed cost of the pipeline.


\begin{table}[htbp]
\caption{Raspberry Pi 3 B+ Profiling of the Base TFLite Backbones (Mean $\pm$ Std over 50 Runs)}
...
\hline
LightTCN-1D & 79,628 & 50.11 $\pm$ 0.11 & 3.25 $\pm$ 0.29 & 1208.28 $\pm$ 266.71 \\
TinyCNN-1D & 116,146 & 11.10 $\pm$ 0.05 & 4.34 $\pm$ 0.21 & 359.75 $\pm$ 73.23 \\
\textbf{ResidualNav-Backbone (Ours)} & \textbf{598,530} & \textbf{25.31 $\pm$ 0.72} & \textbf{3.55 $\pm$ 0.45} & \textbf{671.78 $\pm$ 187.41} \\
ResidualNav-Backbone + RF & 598,530 & 25.61 $\pm$ 0.67 & 3.77 $\pm$ 0.48 & 744.81 $\pm$ 181.95 \\
ShuffleNetV2-1D & 854,618 & 33.36 $\pm$ 0.53 & 3.39 $\pm$ 0.31 & 844.23 $\pm$ 208.12 \\
MobileNetV2-1D & 2,642,562 & 80.08 $\pm$ 1.78 & 3.46 $\pm$ 0.37 & 2067.97 $\pm$ 527.64 \\
RoNIN ResNet (Baseline) & 4,634,882 & 89.42 $\pm$ 6.46 & 3.39 $\pm$ 0.27 & 2269.86 $\pm$ 656.61 \\
\hline
\end{tabular}
}
\end{table}
...
Table~\ref{tab:seen} reports ATE and RTE on seen subjects, with models ordered by increasing neural parameter count. RF correction generally improves trajectory accuracy across lightweight backbones, although the gains vary by architecture. LLIO-Net (ResMLP128)~\cite{llio2022} trails ResidualNav-Backbone by a wide margin despite 18\% fewer parameters (5.3483/3.9878 vs.\ 4.1649/3.1488 ATE/RTE). For ResidualNav-Backbone, RF correction reduces ATE/RTE from 4.1649/3.1488 to 3.5848/2.6081, a 17.2\% RTE reduction, and brings it level with the RoNIN-ResNet baseline (3.7114/2.7614) at $7.7\times$ fewer neural parameters. Among the backbones that meet the 50~ms budget on the Raspberry Pi (Table~\ref{tab:pi_tflite_latency}), ResidualNav-Backbone+RF attains the lowest ATE, while ShuffleNetV2-1D+RF attains a slightly lower RTE (2.4248 versus 2.6081~m). Fig.~\ref{fig:trajectories} shows representative predicted trajectories for four of these seen-subject sequences.

\begin{table}[htbp]
\caption{Seen Test Set Results (Public RoNIN Subset, Mean $\pm$ Std over 32 Sequences)}
\label{tab:seen}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
\hline
LightTCN-1D & 5.6082 $\pm$ 2.94 & 3.3909 $\pm$ 1.70 & 5.7823 $\pm$ 2.83 & 3.4441 $\pm$ 1.47 \\
TinyCNN-1D & 4.7783 $\pm$ 2.17 & 3.1978 $\pm$ 1.62 & 4.7689 $\pm$ 1.94 & 2.7939 $\pm$ 1.41 \\
LLIO-Net (ResMLP128)~\cite{llio2022} & 5.3483 $\pm$ 2.33 & 3.9878 $\pm$ 1.89 & 5.1635 $\pm$ 2.45 & 3.6210 $\pm$ 1.76 \\
ResidualNav-Backbone (Ours) & 4.1649 $\pm$ 1.63 & 3.1488 $\pm$ 1.47 & \textbf{3.5848 $\pm$ 1.34} & \textbf{2.6081 $\pm$ 1.22} \\
ShuffleNetV2-1D & 3.9425 $\pm$ 1.57 & 2.8061 $\pm$ 1.54 & 3.9048 $\pm$ 1.46 & 2.4248 $\pm$ 1.31 \\
MobileNetV2-1D & 3.9705 $\pm$ 1.64 & 2.7708 $\pm$ 1.60 & 3.8009 $\pm$ 1.55 & 2.4261 $\pm$ 1.39 \\
RoNIN ResNet (Baseline) & 3.7114 $\pm$ 1.63 & 2.7614 $\pm$ 1.58 & -- & -- \\
\hline
\end{tabular}
}
\end{table}
...
RoNIN-ResNet baseline (green, dashed) and the proposed ResidualNav-Backbone + RF
...
Table~\ref{tab:unseen} reports generalization to unseen subjects. RF correction yields smaller gains here, reducing ResidualNav-Backbone RTE from 4.9368 to 4.7313 while leaving ATE essentially unchanged (5.8977 to 5.9046). The retrained RoNIN-ResNet baseline (5.1400/4.3770) is stronger on unseen subjects, so the lightweight backbones trade some cross-subject generalization for their reduced footprint, and LLIO-Net degrades further still (7.0285/5.6777). For reference, the original RoNIN study~\cite{ronin2019} reported ResNet ATE/RTE of 3.54/2.67 on seen subjects and 5.14/4.37 on unseen subjects using the full training dataset; our retrained values (3.7114/2.7614 seen, 5.1400/4.3770 unseen) are close to these published numbers, supporting the validity of the evaluation protocol despite using only the publicly accessible subset (Table~\ref{tab:dataset_splits}). MobileNetV2-1D+RF attains the lowest error among the lightweight models, but its backbone alone needs 80.08~ms per window on the Raspberry Pi, exceeding the 50~ms budget, whereas ResidualNav-Backbone needs 25.31~ms. Accuracy therefore matches the full-size baseline on seen subjects ($7.7\times$ fewer parameters) but only partially on unseen ones.

\begin{table}[htbp]
\caption{Unseen Test Set Results (Generalization, Mean $\pm$ Std over 32 Sequences)}
\label{tab:unseen}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
\textbf{ATE (+RF)} & \textbf{RTE (+RF)} \\
\hline
LightTCN-1D & 8.6503 $\pm$ 6.67 & 4.9925 $\pm$ 2.59 & 8.6435 $\pm$ 6.30 & 4.7760 $\pm$ 2.38 \\
TinyCNN-1D & 6.5321 $\pm$ 3.88 & 5.0162 $\pm$ 2.41 & 6.4391 $\pm$ 3.46 & 4.5282 $\pm$ 2.20 \\
LLIO-Net (ResMLP128)~\cite{llio2022} & 7.0285 $\pm$ 4.23 & 5.6777 $\pm$ 2.90 & 6.5397 $\pm$ 3.83 & 5.2421 $\pm$ 2.63 \\
ResidualNav-Backbone (Ours) & 5.8977 $\pm$ 3.22 & 4.9368 $\pm$ 2.08 & \textbf{5.9046 $\pm$ 3.52} & \textbf{4.7313 $\pm$ 1.97} \\
ShuffleNetV2-1D & 5.7805 $\pm$ 2.88 & 4.6046 $\pm$ 2.01 & 5.9522 $\pm$ 3.27 & 4.1057 $\pm$ 1.82 \\
MobileNetV2-1D & 5.5274 $\pm$ 3.32 & 4.5530 $\pm$ 2.05 & 5.2699 $\pm$ 3.25 & 4.0811 $\pm$ 1.83 \\
RoNIN ResNet (Baseline) & 5.1400 $\pm$ 3.26 & 4.3770 $\pm$ 2.07 & -- & -- \\
\hline
\end{tabular}
}
\end{table}
...
RF correction consistently yields larger improvements in RTE than ATE across most backbones, since the corrector operates on short-horizon velocity residuals before trajectory integration and mainly reduces local drift rather than global error. This suggests lightweight backbones share systematic short-term prediction errors that compact kinematic and rolling inertial features can mitigate.

These results test whether lightweight backbones share a common
residual structure; transfer proves real but not universal: it
improves seen-subject ATE and RTE for TinyCNN-1D, ShuffleNetV2-1D and
MobileNetV2-1D, but worsens both for LightTCN-1D (5.6082/3.3909 to
5.7823/3.4441~m, Table~\ref{tab:seen}). On unseen subjects RTE gains
persist for most backbones while ATE is flat or occasionally worse
(e.g., ShuffleNetV2-1D, 5.7805 to 5.9522~m, Table~\ref{tab:unseen}).

\subsection{Stage-2 Corrector Comparison}

Table~\ref{tab:corrector_variants} compares four residual correctors on
ResidualNav-Backbone: (A) the Random Forest used in ResidualNav,
(B) an exponential moving average, (C) a small MLP, and (D) a TCN. On
seen subjects Random Forest gives the largest ATE (4.1649 to 3.5848)
and RTE (3.1488 to 2.6081) reduction, with the MLP second. On unseen
subjects differences are small: the MLP attains a marginally lower ATE
(5.8741 vs.\ 5.9046), while Random Forest attains the lowest RTE
(4.7313), motivating its use in ResidualNav.

\begin{table}[htbp]
\caption{Stage-2 Residual Corrector Comparison on ResidualNav-Backbone (ATE / RTE, m, Mean $\pm$ Std over 32 Sequences). Best value per column in bold.}
...
\hline
None (base)      & 4.1649 $\pm$ 1.63 & 3.1488 $\pm$ 1.47 & 5.8977 $\pm$ 3.22 & 4.9368 $\pm$ 2.08 \\
A: Random Forest & \textbf{3.5848 $\pm$ 1.34} & \textbf{2.6081 $\pm$ 1.22} & 5.9046 $\pm$ 3.52 & \textbf{4.7313 $\pm$ 1.97} \\
B: EMA           & 4.1575 $\pm$ 1.63 & 3.1220 $\pm$ 1.48 & 5.8889 $\pm$ 3.22 & 4.9136 $\pm$ 2.08 \\
C: MLP           & 3.7298 $\pm$ 1.52 & 2.8269 $\pm$ 1.25 & \textbf{5.8741 $\pm$ 3.28} & 4.8625 $\pm$ 1.98 \\
D: TCN           & 4.8027 $\pm$ 2.29 & 2.8996 $\pm$ 1.57 & 6.7961 $\pm$ 3.68 & 4.8896 $\pm$ 1.90 \\
\hline
\end{tabular}%
}
\end{table}
...
Table~\ref{tab:crossmodel_rf_result} shows ResidualNav improves all NeurIT metrics (ATE/RTE 1.3578/0.6958 to 1.2618/0.6791, plus lower PDE/AYE) and RIDI (2.1924/3.6724 to 2.0892/3.3580), with larger RTE gains in both, consistent with mitigating short-term drift.
...
Table~\ref{tab:crossmodel_inference_time} summarizes the deployment overhead: ResidualNav adds 1.628~ms per processed output for NeurIT and only 0.023~ms for RIDI, reflecting their different output granularities: NeurIT produces fewer, richer outputs, while RIDI streams many lightweight predictions. Because accuracy gains hold and overhead stays bounded on both systems, ResidualNav can be integrated as a lightweight postprocessing stage across inertial-navigation pipelines.
...
We presented a performance-aware inertial odometry framework: a two-stage pipeline combining ResidualNav-Backbone, a lightweight 1D backbone, with ResidualNav, a supervised Random Forest corrector reducing local velocity error before trajectory integration. On RoNIN, this design lowers RTE over the uncorrected backbone on seen and unseen subjects, matches RoNIN-ResNet on seen subjects with 87.1\% fewer neural parameters, and transfers to NeurIT and RIDI. On a Raspberry Pi 3 B+, the backbone runs in 25.31~ms per window, within the 50~ms budget, $3.5\times$ faster and $3.4\times$ more energy-efficient than RoNIN-ResNet. Two lightweight alternatives edge out ResidualNav-Backbone+RF on seen-subject RTE: MobileNetV2-1D+RF needs $4.4\times$ more parameters and exceeds the budget outright at 80.08~ms; ShuffleNetV2-1D+RF meets the budget (33.36~ms) but trades a worse ATE (3.9048 vs.\ 3.5848~m) and $1.43\times$ the parameters. These results argue for evaluating edge inertial odometry as a complete system.
