...
\author{
\IEEEauthorblockN{Choudam Jaitej\textsuperscript{*}, Akshay Toshniwal\textsuperscript{*}, Sohel Samirkhan Modi\textsuperscript{*} and Priyanka Bagade}
\IEEEauthorblockA{
Department of Computer Science and Engineering\\
Indian Institute of Technology Kanpur, India\\
jaitejc25@iitk.ac.in, akshayt24@iitk.ac.in, sohelsm24@iitk.ac.in, pbagade@iitk.ac.in
}
}
...
% ============================================================
\begin{abstract}
% Deep inertial odometry can estimate pedestrian trajectories from commodity IMU streams, but accurate models remain expensive for edge devices because they require millions of parameters and their per-window velocity predictions can accumulate into long-horizon drift. This paper studies inertial odometry as an accuracy resource tradeoff and proposes a two-stage edge-oriented pipeline. First, a lightweight YOLOv26-1D backbone regresses planar velocity from 200~Hz IMU windows using 1.17M parameters, a 74.6\% reduction relative to the 4.63M-parameter RoNIN-ResNet baseline. Second, a multi-output Random Forest residual corrector termed \textit{ResidualNav} which predicts local velocity errors from compact kinematic and inertial features before trajectory integration. To avoid data-access asymmetry, all neural models are trained from scratch on the same public RoNIN subset and evaluated on both seen and unseen subjects. The proposed hybrid reaches near-parity in Absolute Trajectory Error while improving Relative Trajectory Error by 4.49\% on seen subjects and 4.88\% on unseen subjects. These results show that a small backbone combined with structured residual correction can preserve trajectory quality while substantially reducing the model footprint needed for real-time edge navigation.
...
alone, making it attractive for smartphones, wearables and other
edge devices that must operate without GPS or cameras \cite{ronin2019}. Classical physics-based methods recover position by double-integrating linear acceleration \cite{woodman2007,titterton2004}. However, uncorrected sensor biases cause position errors to diverge rapidly \cite{woodman2007}, making pure double integration impractical without external constraints such as zero-velocity updates \cite{foxlin2005} or step counting \cite{jimenez2009}. These constraints often assume specific phone placements or motion styles and can fail during unconstrained pedestrian motion \cite{ridi2018}.

Data-driven inertial odometry reduces some of these assumptions by
learning a direct mapping from IMU windows to velocity. RIDI
\cite{ridi2018} showed that regression-based models can estimate velocity from inertial streams, IONet \cite{ionet2018} introduced recurrent networks and RoNIN
\cite{ronin2019} established a benchmark with 100 subjects and several
neural backbones, including ResNet-18 \cite{resnet2016}, LSTM
\cite{lstm1997} and TCN \cite{tcn2018}. Among these, RoNIN-ResNet is a strong accuracy baseline. However, its 4.63M parameters and independent window-by-window predictions
raise two deployment concerns: memory and compute cost on edge
processors, and short-range velocity jitter that can accumulate into
trajectory drift. This makes inertial odometry a systems performance problem in which trajectory quality, latency, and model footprint must be evaluated together.

This paper studies that tradeoff by asking how much model footprint and inference cost can be reduced while preserving trajectory quality under a real-time IMU processing budget. Our approach splits the task into two stages: a compact predictor estimates motion, and ResidualNav learns recurring residual errors before velocity is integrated into position. The goal is to identify a more favorable operating point for real-time edge navigation, rather than simply optimizing prediction accuracy in isolation. The main contributions are:
% \begin{itemize}
%   \item We adapt a YOLOv26-inspired 1D convolutional network as a
%         lightweight IMU-to-velocity backbone, reducing the parameter
%         count from 4.63M to 1.17M and surpassing the ResNet baseline
%         in RTE on both seen and unseen test subjects after RF correction.
%   \item We introduce \textit{ResidualNav}, a supervised residual-correction layer implemented as
%       a multi-output Random Forest regressor that learns to predict and
%       cancel short-horizon velocity errors, substantially reducing
%       cumulative integration drift.
%   \item We evaluate both accuracy and deployability, besides trajectory
%         errors, we report how large the model is, how often it must
%         produce an output and whether its inference time fits the
%         real-time IMU processing budget.
%   \item We benchmark TinyCNN, LightTCN, ShuffleNetV2, MobileNetV2,
%         YOLOv26 and RoNIN-ResNet on Raspberry Pi to show that the
%         proposed residual-correction process remains robust across
%         lightweight edge backbones.
%   \item We apply the same RF postprocessing idea to NeurIT and RIDI,
%         showing that the correction can work across different datasets,
%         odometry models and estimation methods.
% \end{itemize}
...
Data-driven inertial odometry replaces hand-crafted motion heuristics with learned mappings from IMU streams to velocity. RIDI \cite{ridi2018} showed that Support Vector Regression can estimate velocity from stabilized IMU data, including motions that break step-counting methods. IONet \cite{ionet2018} extended this idea with recurrent models, and RoNIN \cite{ronin2019} established a large-scale benchmark with several neural backbones, including ResNet-18, LSTM, and TCN. Later methods tightened the coupling between learned motion and filtering (TLIO \cite{tlio2020}), exploited rotation equivariance (RIO \cite{rio2022}), and introduced attention-based backbones (CTIN \cite{ctin2022}, A2DIO \cite{a2dio2022}). More recent work such as AirIMU \cite{airimu2023} incorporates learned uncertainty propagation into IMU preintegration, enabling tighter coupling with sensor fusion systems, while Deep IMU Bias \cite{deepbias2022} uses LSTM networks to model sensor bias directly within visual-inertial odometry pipelines. While these methods improved trajectory accuracy, they also highlighted a practical deployment issue: better performance often comes with larger models and higher inference cost.

\subsection{Postprocessing and Drift Correction in INS}
Several strategies have been proposed to reduce the drift that inevitably builds up in trajectory estimates. Classical filter-based corrections, such as the
extended Kalman filter, rely on linearized motion models that break down
under aggressive or unconstrained motion \cite{kalman1960}. Hybrid
approaches that combine neural predictions with optimization-based
trajectory smoothing can improve consistency \cite{ronin2019} but they often add
significant system complexity. In contrast, residual correction offers a simpler alternative by learning structured errors left by a primary predictor. This idea motivates our use of a lightweight Random Forest postprocessor to reduce short-horizon velocity error before trajectory integration \cite{solin2018,breiman2001,rf_imu_2023}.
...
compression or architectures designed for efficiency from the beginning.
Knowledge distillation, pruning and quantization are common
post-training strategies, but they can be sensitive to calibration data
and teacher-model quality. Architecturally efficient designs such as
MobileNet \cite{mobilenet2017}, ShuffleNet \cite{shufflenet2018} and
the YOLO family achieve parameter efficiency through depthwise separable
convolutions, channel shuffling, cross-stage partial connections and
compact feature pyramids.
Lightweight variants of YOLO in particular have been adapted for
navigation-adjacent tasks including real-time indoor object detection
on mobile platforms \cite{yolonav2026}.
For temporal sensing, lightweight 1D convolutional models and TCNs \cite{ronin2019} have been explored as efficient alternatives to heavier recurrent networks, and LLIO \cite{llio2022} likewise pursues a lightweight learned inertial odometer. These results support our goal of combining a compact backbone with residual correction to obtain a better accuracy--latency--footprint tradeoff for edge inertial odometry.
...
trajectory. Each inference step consumes a one-second IMU window with
...
\begin{tikzpicture}[
  ...
  % Plain data nodes (no fill, just border)
  databox/.style={rectangle, rounded corners=3pt, draw=gray!70, thick,
                  fill=blue!7, minimum width=1.8cm, minimum height=0.75cm,
                  font=\small, align=center},
  % Model nodes (light coloured, bold)
  modelbox/.style={rectangle, rounded corners=3pt, draw=blue!60, very thick,
                  fill=blue!15, minimum width=2.0cm, minimum height=0.85cm,
                  font=\small\bfseries, align=center},
  % RF node (green tint)
  rfbox/.style={rectangle, rounded corners=3pt, draw=green!60!black, very thick,
                  fill=green!12, minimum width=2.0cm, minimum height=0.85cm,
                  font=\small\bfseries, align=center},
  % Correction block
  corrbox/.style={rectangle, rounded corners=3pt, draw=orange!70, very thick,
                  fill=orange!10, minimum width=2.1cm, minimum height=0.85cm,
                  font=\small, align=center},
  % Arrow style
  arrow/.style={-{Stealth[length=6pt]}, thick, draw=gray!80},
  % Dashed arrow
  darrow/.style={-{Stealth[length=5pt]}, thick, dashed, draw=gray!60},
  % Small label style
  lbl/.style={font=\scriptsize\itshape, text=gray!80, align=center}
]

%% ── NODE 1: Person icon + IMU data ──────────────────────────────────────────
\node[inner sep=0pt] (person)
  {\includegraphics[width=1.1cm]{walk.png}};   % replace with your person icon
\node[lbl, below=0.08cm of person] (imutext)
  {Accelerometer Data\\Gyroscope Data};

%% ── NODE 2: YOLOv26-1D ───────────────────────────────────────────────────────
\node[modelbox, right=1.6cm of person] (yolo)
  {YOLOv26-1D-Eff\\Backbone};
\node[lbl, below=0.1cm of yolo] {0.60M params};

%% ── NODE 3: Random Forest ────────────────────────────────────────────────────
\node[rfbox, right=2.0cm of yolo] (rf)
  {Random Forest\\Corrector};

%% ── NODE 4: Correction block ─────────────────────────────────────────────────
\node[corrbox, right=2.0cm of rf] (corr)
  {$\tilde{\mathbf{v}}_t = \hat{\mathbf{v}}_t + \Delta\mathbf{v}_t$};
\node[lbl, below=0.1cm of corr] {Drift-reduced};

%% ── NODE 5: Trajectory image ─────────────────────────────────────────────────
\node[inner sep=2pt, draw=gray!50, rounded corners=3pt,
      right=1.4cm of corr] (traj)
  {\includegraphics[width=2.2cm, height=2.2cm]{trajectory.png}};
\node[lbl, below=0.05cm of traj] {Trajectory};

%% ── ARROWS ───────────────────────────────────────────────────────────────────

% Person → YOLOv26
\draw[arrow] (person.east) -- (yolo.west)
  node[midway, above, font=\scriptsize, align=center]
  {IMU Window\\$[200{\times}6]$};

% YOLOv26 → RF
\draw[arrow] (yolo.east) -- (rf.west)
  node[midway, above, font=\scriptsize, align=center]
  {$\hat{\mathbf{v}}_t \in \mathbb{R}^2$};

% RF → Correction
\draw[arrow] (rf.east) -- (corr.west)
  node[midway, above, font=\scriptsize, align=center]
  {$\hat{\mathbf{r}}_t$\\(residual)};

% Correction → Trajectory
\draw[arrow] (corr.east) -- (traj.west)
  node[midway, above, font=\scriptsize, align=center]
  {$\tilde{\mathbf{v}}_t$};

% Dashed: Person IMU features → RF (below the main flow)
\draw[darrow] (person.south) -- ++(0,-0.6)
  -| node[near start, below, font=\scriptsize\itshape, text=gray]
  {IMU features $\mathbf{x}_t$} (rf.south);
...
The deepest map is then refined by an attention block applied over
those seven tokens before any fusion takes place. Stacked convolutions
alone already give this stage a theoretical receptive field of 275
samples, wider than the 200-sample window; the corresponding effective
receptive field, however, is far narrower, since propagating gradients
from a central token back to the input concentrates 90\% of the
sensitivity within about 97 samples on average over 20 random
initializations. Self-attention supplies the missing coupling directly,
letting any pair of tokens interact in a single content-dependent step
instead of through a distance-decayed stack of convolutions. Because
only seven tokens are attended, the block does not need full-width
projections: a depthwise 3-tap convolution first performs local
temporal mixing, the tokens are then layer-normalized and projected
from 256 to 96 dimensions for a four-head self-attention and projected
back, and a layer-normalized feed-forward network of half the channel
width follows, each sub-block with an identity residual connection.
...
At each timestep $t$, the base model predicts planar velocity $\hat{\mathbf{v}}_t$:
\begin{equation}
  \hat{\mathbf{v}}_t = \begin{bmatrix} \hat{v}_{x,t} \\ \hat{v}_{y,t} \end{bmatrix} \in \mathbb{R}^2
\end{equation}
and the corresponding ground-truth velocity $\mathbf{v}_t$ is:
\begin{equation}
  \mathbf{v}_t = \begin{bmatrix} v_{x,t} \\ v_{y,t} \end{bmatrix}
\end{equation}
with the residual (prediction error) $\mathbf{r}_t$:
\begin{equation}
  \mathbf{r}_t = \mathbf{v}_t - \hat{\mathbf{v}}_t
\end{equation}
The postprocessor learns a function $g_\phi(\cdot)$ such that:
\begin{equation}
  \hat{\mathbf{r}}_t = g_\phi(\mathbf{x}_t)
\end{equation}
where $\mathbf{x}_t$ is a handcrafted feature vector built from
predicted velocity behavior and IMU-derived local context.
The corrected velocity is then:
\begin{equation}
  \tilde{\mathbf{v}}_t = \hat{\mathbf{v}}_t + \alpha \hat{\mathbf{r}}_t
\end{equation}
with optional correction clipping for robustness:
\begin{equation}
  \Delta \mathbf{v}_t = \mathrm{clip}\!\left(\alpha \hat{\mathbf{r}}_t,\,-c,\,+c\right),
  \quad \tilde{\mathbf{v}}_t = \hat{\mathbf{v}}_t + \Delta \mathbf{v}_t
\end{equation}
Here $\alpha$ is the correction gain and $c$ is the residual clip bound,
both tuned on the validation set.

\subsubsection{Drift Reduction Rationale}

Trajectory is obtained by integrating velocity over time:
\begin{equation}
  \hat{\mathbf{p}}_{t+1} = \hat{\mathbf{p}}_t + \hat{\mathbf{v}}_t \Delta t
\end{equation}
A small per-step velocity bias $\boldsymbol{\epsilon}_t$ accumulates
over the full trajectory horizon $T$:
\begin{equation}
...
which in turn reduces the drift reflected in trajectory prediction errors over long
paths. Improving short-term velocity predictions is therefore a
...
\begin{itemize}
  ...
  \item \textbf{Normalized trajectory progress:}
        \begin{equation}
          \tau_t = \frac{t - t_0}{t_1 - t_0}
        \end{equation}
\end{itemize}
...
minimize:
\begin{equation}
  \min_\phi \sum_t \left\| \mathbf{r}_t - g_\phi(\mathbf{x}_t) \right\|_2^2
\end{equation}
...
\begin{enumerate}
  \item Run the YOLOv26-1D-Eff backbone to obtain $\hat{\mathbf{v}}_t$
        for all timesteps.
  \item Construct the RF training dataset with inputs $\mathbf{x}_t$
        and labels $\mathbf{r}_t = \mathbf{v}_t - \hat{\mathbf{v}}_t$
        (requires ground truth, used only at train time).
  \item Train the RF with validation-guided hyperparameter tuning.
...
\subsubsection{Method Positioning}

The proposed system is a \emph{neural + nonparametric residual
correction framework}, with \textit{ResidualNav} serving as the local refinement stage. The neural backbone performs global
inertial-to-velocity mapping via learned temporal convolution features,
while the RF postprocessor performs local residual denoising and bias
correction using interpretable engineered features. The two components
are complementary: the backbone provides the global velocity estimate;
the RF provides the local refinement that the convolutional
head cannot capture.
...
All experiments use the RoNIN benchmark \cite{ronin2019}, a
publicly available inertial navigation dataset containing over 42.7
hours of IMU and ground-truth 3D motion data from 100 human subjects
in three indoor buildings, collected using three
Android smartphone models. Ground-truth 3D trajectories are provided
by a body-mounted 3D tracking phone (Asus Zenfone AR) with a
spatial drift of less than 0.3~m per 10 minutes. Both IMU sensor
data and pose data are recorded at 200~Hz.

Because not every RoNIN trajectory is publicly redistributed, all
models are trained and evaluated on the same publicly accessible
portion of the benchmark, summarized in Table~\ref{tab:dataset_splits}:
152 sequences from 57 subjects totaling 24.87~hours, roughly 58\% of
the full release, which yields about 1.79M input windows at stride 10.
The \emph{seen} test set holds out sequences from subjects that also
contribute training data, whereas the \emph{unseen} test set uses 15
subjects appearing in no training or validation sequence; we verified
at the subject level that the unseen split shares no subject with the
other three, so those results carry no subject-level leakage. Training
every model on this common subset removes data-access asymmetry: the
goal is a controlled comparison of lightweight architectures under
matched data and evaluation protocol, not a reproduction of absolute
...
\midrule
Training server & CPU & Dual Intel Xeon Gold 6226R, 2.90~GHz \\
Training server & CPU topology & 2 sockets, 32 physical cores, 64 logical CPUs \\
Training server & Memory/GPU & 376~GiB RAM, 4 NVIDIA GPUs, $\sim$48~GiB VRAM/GPU \\
Training server & Software & NVIDIA driver 580.65.06, CUDA driver 13.0 \\
\midrule
Edge device & Board & Raspberry Pi 3 Model B+ Rev. 1.3 \cite{rpi3bplus} \\
Edge device & CPU & ARM Cortex-A53/BCM2835, 4 cores, up to 1.4~GHz \\
Edge device & Memory & 920~MiB RAM, 199~MiB swap \\
Edge device & OS/storage & Raspbian GNU/Linux 12, SanDisk 16~GB SD card \\
\bottomrule
\end{tabular}
}
\end{table}
...
\cite{tflite} and executed on the Raspberry Pi with batch size one, a
200-sample IMU window and step size 10. Each model is profiled over 50
runs; each run processes one seen-subject test sequence drawn at random
from a pool of nine. During each run the board supply voltage and
current are logged with \TBD{power meter}, giving the average board
power and the measured energy consumed over the run; latency, power and
energy are averaged over the 50 runs. Profiling covers the base TFLite backbones only;
the RF corrector is not included. The benchmark therefore captures low-power ARM
latency and energy directly rather than workstation-side timing.
...
\begin{itemize}
  \item \textbf{Absolute Trajectory Error (ATE):}
        ATE is the root-mean-square error between the estimated and
        ground-truth trajectories over the whole sequence, computed in
        the common global frame without further alignment, as in
        RoNIN~\cite{ronin2019}. It reflects the global accuracy of the
        complete predicted path.

  \item \textbf{Relative Trajectory Error (RTE):}
        RTE is the root-mean-square error over one-minute intervals; for
        sequences shorter than one minute, the positional error at the
        last frame is scaled proportionally, following
        RoNIN~\cite{ronin2019}. It captures local drift and shows how
        well the model preserves short-term motion consistency.

  \item \textbf{Position Drift Error (PDE):}
        PDE measures the final position drift with respect to the total
        traveled path length. It indicates how much the estimated
        endpoint deviates from the true endpoint relative to the length
        of the trajectory.

  \item \textbf{Absolute Yaw Error (AYE):}
        AYE is the root-mean-square difference between the heading
        directions of the estimated and ground-truth
        trajectories~\cite{neurit2024}. It evaluates how accurately the
        model preserves the direction of movement along the path.

\end{itemize}


To characterize deployment suitability, we also report system-performance
metrics from Section~\ref{sec:sysmodel}. These are separate from
trajectory-accuracy metrics.

\begin{itemize}
  \item \textbf{Parameter Count:} number of trainable neural weights,
        used as a hardware-independent measure of model complexity.

  \item \textbf{FP32 Weight Memory:} storage required when each learned
        weight is kept as a 32-bit floating-point value, i.e., 4 bytes:
        \begin{equation}
        \mathrm{FP32\ size\ (MiB)} =
        \frac{4 \times \#\mathrm{parameters}}{1024^2}
        \end{equation}
        This provides a common memory baseline before quantization or
        hardware-specific compression.

  \item \textbf{Model-Footprint Reduction:} percentage decrease in model
        size relative to RoNIN-ResNet:
        \begin{equation}
        \mathrm{Reduction} =
        \frac{M_{\mathrm{base}}-M_{\mathrm{model}}}
        {M_{\mathrm{base}}}\times 100\%
        \end{equation}

  \item \textbf{Update Budget:} maximum time available for each new
        prediction. With 200~Hz IMU data and stride 10:
        \begin{equation}
        T_{\mathrm{upd}}=\frac{10}{200}=0.05~\mathrm{s}=50~\mathrm{ms}
        \end{equation}

  \item \textbf{Inference Latency and Overhead:} the runtime needed by the
        model and the RF postprocessor. A real-time implementation should
        satisfy
        \begin{equation}
        T_{\mathrm{inf}} + T_{\mathrm{RF}} \leq T_{\mathrm{upd}}
        \end{equation}
\end{itemize}

\subsection{Baseline and Ablation Models}

In addition to the RoNIN-ResNet baseline and the proposed
YOLOv26-1D-Eff pipeline, we evaluate four competing lightweight 1D
backbones to test whether the residual-correction process remains
effective across architectural choices:
\begin{enumerate}
  ...
  \item \textbf{Competing lightweight backbones:} TinyCNN-1D,
        LightTCN-1D, ShuffleNetV2-1D and MobileNetV2-1D, each trained
        under the same RoNIN protocol and evaluated with and without RF
        correction. For these backbones the corrector is not refitted:
        a single Random Forest, fitted to the residuals of a
        1.17M-parameter YOLOv26-1D reference backbone, is applied to
        each of them unchanged, so their corrected results measure how
        well ResidualNav transfers across backbones.
\end{enumerate}
...
YOLOv26-1D-Eff is trained with Adam, no weight decay and a head
dropout of 0.5 for up to 100 epochs. RoNIN-ResNet and the competing
backbones are trained with MuSGD, a momentum-SGD variant whose
updates are orthogonalized by Newton--Schulz iteration, using
momentum 0.9 and weight decay $10^{-4}$ (head dropout 0.2 for the
competing backbones), for up to 100 epochs (RoNIN-ResNet,
ShuffleNetV2-1D, MobileNetV2-1D) or 200 epochs (TinyCNN-1D,
LightTCN-1D). Every model reached a validation plateau well before
its budget. Although RoNIN trained its
ResNet with Adam, our MuSGD retrain reproduces its published
unseen-subject accuracy (5.14/4.38 versus 5.14/4.37~m), indicating
that the baseline is not disadvantaged by the optimizer.
...
\begin{minipage}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=0.95\linewidth]{traj1_large_text.png}
  % \subcaption{Sequence 1 (Seen subject): YOLOv26+RF achieves lower ATE and
  % RTE than ResNet despite using 75\% fewer parameters, with the predicted
  % trajectory closely following the ground truth layout.}
  \label{fig:traj1}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=0.95\linewidth]{traj2_large_text.png}
  % \subcaption{Sequence 2 (Seen subject): The hybrid model produces a
  % trajectory that adheres more tightly to the ground truth shape,
  % demonstrating the residual corrector's effectiveness in suppressing
  % step-wise velocity drift.}
  \label{fig:traj2}
\end{minipage}
...
\begin{minipage}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=0.95\linewidth]{traj3_large_text.png}
  % \subcaption{Sequence 3 (Seen subject): Even on a geometrically complex
  % looped trajectory, YOLOv26+RF maintains competitive path fidelity,
  % confirming that the Random Forest corrector generalizes across diverse
  % motion patterns within the training distribution.}
  \label{fig:traj3}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=0.95\linewidth]{traj4_large_text.png}
  \label{fig:traj4}
\end{minipage}
...
% Table~\ref{tab:model_footprint} separates neural model size from RF
% corrector complexity. YOLOv26-1D uses 1,174,594 neural parameters,
% 74.6\% fewer than RoNIN-ResNet, while ResidualNav is a 300-tree RF
% corrector over 18 input features. RF parameters are not trainable
% parameters, so they do not contribute to the total neural-parameter
% count or the FP32 neural-weight size.

Table~\ref{tab:model_footprint} separates neural backbone footprint from residual-correction complexity. YOLOv26-1D-Eff contains 0.60M trainable parameters, an 87.1\% reduction relative to RoNIN-ResNet. ResidualNav employs a 400-tree Random Forest over 18 input features; as a non-neural component, its storage cost is reported separately and is not included in the neural parameter count or FP32 weight size.
% Because this RF state is not a neural
% weight matrix, its tree structure and storage footprint are measured
% separately from the neural FP32 size.
...
% \midrule
% YOLOv26-1D & 1,174,594 & 4.48 MiB  \\
% \textbf{YOLOv26-1D + RF} & \textbf{1,174,594} & \textbf{4.48 MiB}  \\
...
Table~\ref{tab:pi_tflite_latency} reports the per-window latency, average board power and measured energy per run of the TFLite backbones on the Raspberry Pi 3 B+. YOLOv26-1D-Eff runs in 25.31~ms per window, within the 50~ms update budget with 24.7~ms of headroom, and is $3.5\times$ faster than RoNIN-ResNet (89.42~ms) and $3.2\times$ faster than MobileNetV2-1D (80.08~ms), both of which exceed the budget. Average board power varies little across models (3.39--4.34~W), so energy is governed mainly by latency: YOLOv26-1D-Eff consumes 671.78~J per run against 2269.86~J for RoNIN-ResNet, a $3.4\times$ reduction. TinyCNN-1D and ShuffleNetV2-1D also meet the budget, but after correction TinyCNN-1D is less accurate on both metrics and ShuffleNetV2-1D has a higher ATE (Table~\ref{tab:seen}).
...
\midrule
LightTCN-1D & 79,628 & \TBD{--} & \TBD{--} & \TBD{--} \\
...
Table~\ref{tab:unseen} reports generalization to unseen subjects. RF correction yields smaller gains here, reducing YOLOv26-1D-Eff RTE from 4.9368 to 4.7313 while leaving ATE essentially unchanged (5.8977 to 5.9046). The retrained RoNIN-ResNet baseline (5.1400/4.3770) is stronger on unseen subjects, so the lightweight backbones trade some cross-subject generalization for their reduced footprint; this retrained baseline stays close to the original RoNIN results, supporting the validity of the evaluation protocol. MobileNetV2-1D+RF attains the lowest error among the lightweight models, but its backbone alone needs 80.08~ms per window on the Raspberry Pi, exceeding the 50~ms budget, whereas the YOLOv26-1D-Eff backbone needs 25.31~ms.
...
Table~\ref{tab:corrector_variants} compares five residual correctors on
the YOLOv26-1D-Eff backbone: (A) the Random Forest used in ResidualNav,
(B) ridge regression, (C) an exponential moving average, (D) a small
MLP, and (E) a temporal convolutional network. On seen subjects the
Random Forest gives the largest reduction in both ATE (4.1649 to 3.5848)
and RTE (3.1488 to 2.6081), with the MLP second. On unseen subjects the
differences between correctors are small: ridge regression is marginally
best (5.7980/4.7197) and the Random Forest remains within 0.012~m of it
in RTE. The nonlinear Random Forest therefore provides the strongest
correction where residual structure is shared with the training
subjects, which motivates its use in ResidualNav.
...
A: Random Forest & \textbf{3.5848} & \textbf{2.6081} & 5.9046 & 4.7313 \\
B: Ridge         & 3.9472 & 2.9097 & \textbf{5.7980} & \textbf{4.7197} \\
C: EMA           & 4.1575 & 3.1220 & 5.8889 & 4.9136 \\
D: MLP           & 3.7298 & 2.8269 & 5.8741 & 4.8625 \\
E: TCN           & 4.8027 & 2.8996 & 6.7961 & 4.8896 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[htbp]
\caption{Residual Correction Results for NeurIT}
\label{tab:neurit_rf_result}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
% Table~\ref{tab:neurit_rf_result} shows that ResidualNav improves every
% NeurIT metric, reducing ATE/RTE by 7.1\%/2.4\% while also lowering PDE
% and AYE. Fig.~\ref{fig:neurit_rf_comparison} illustrates the improvement
% on the 101office1 sequence from their dataset.

Table~\ref{tab:neurit_rf_result} shows that ResidualNav improves all NeurIT metrics, reducing ATE and RTE from 1.3578/0.6958 to 1.2618/0.6791 while also lowering PDE and AYE. Similarly, Table~\ref{tab:ridi_rf_result} shows improvements for RIDI, reducing ATE/RTE from 2.1924/3.6724 to 2.0892/3.3580. The larger RTE reductions observed in both systems are consistent with the role of residual correction in mitigating short-term drift before trajectory integration. Figures~\ref{fig:neurit_rf_comparison} and~\ref{fig:ridi_rf_comparison} provide representative trajectory examples.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.90\columnwidth]{101office1.png}
\caption{NeurIT trajectory comparison before and after ResidualNav RF
postprocessing on the 101office1 sequence.}
\label{fig:neurit_rf_comparison}
\end{figure}

\begin{table}[htbp]
\caption{Inference-Time Summary for NeurIT}
\label{tab:neurit_inference_time}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
% Table~\ref{tab:neurit_inference_time} shows a bounded RF overhead of
% 0.573~s per sequence, or 1.628~ms per processed output. This includes
% feature construction, output alignment, RF inference and correction.
% NeurIT's nine-channel sequence-to-sequence TF-BRT uses overlapping
% windows and time-frequency and recurrent processing; hence, each output
% contains rich temporal context and its postprocessing cost is amortized
% over relatively few outputs.

Tables~\ref{tab:neurit_inference_time} and~\ref{tab:ridi_inference_time} summarize the deployment overhead. ResidualNav adds 1.628~ms per processed output for NeurIT and only 0.023~ms for RIDI, including feature construction, RF inference, and trajectory correction. The difference reflects the distinct output granularities of the two pipelines: NeurIT produces fewer, temporally richer outputs, whereas RIDI generates a dense stream of lightweight predictions. Despite these differences, the additional runtime remains small relative to the base pipeline costs.

\begin{table}[htbp]
\caption{Residual Correction Results for RIDI}
\label{tab:ridi_rf_result}
\centering
\scriptsize
\begin{tabular}{lcc}
...
% Table~\ref{tab:ridi_rf_result} shows ATE/RTE reductions of 4.7\%/8.6\% for RIDI. The larger RTE gain indicates effective correction of
% short-term residuals before integration, also visible in
% Fig.~\ref{fig:ridi_rf_comparison}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{ridi_rf_comparison_large_text.png}
\caption{RIDI trajectory comparison before and after ResidualNav RF
postprocessing.}
\label{fig:ridi_rf_comparison}
\end{figure}

\begin{table}[htbp]
\caption{Inference-Time Summary for RIDI}
\label{tab:ridi_inference_time}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
...
% Table~\ref{tab:ridi_inference_time} reports only 0.390~s RF overhead per
% RIDI sequence, or 0.023~ms per processed output. RIDI evaluates costly
% regression and correction operations at selected frame intervals, and
% its dense output stream amortizes postprocessing over many outputs.
% The measured timings correspond to approximately 352 NeurIT outputs
% and 19,000 RIDI outputs per evaluated sequence. Thus, NeurIT has lower
% sequence time but higher per-output latency because it processes fewer,
% richer outputs; RIDI processes many inexpensive outputs. Since their
% outputs have different temporal granularities, per-output latency is
% pipeline-specific rather than an equal-work comparison. Overall, the
% consistent gains with bounded overhead show that ResidualNav transfers
% across neural and classical odometry pipelines, datasets and sensor
% configurations.

Overall, the consistent accuracy improvements and bounded overhead across both neural and classical systems suggest that ResidualNav can be integrated as a lightweight postprocessing stage across diverse inertial-navigation pipelines, datasets, and sensing configurations.



\subsection{Comparison with RoNIN Original Benchmark}

For reference, the original RoNIN study~\cite{ronin2019} reported ResNet ATE/RTE values of 3.54/2.67 on seen subjects and 5.14/4.37 on unseen subjects using the full training dataset. Our ResNet retraining on the publicly available subset (Table~\ref{tab:dataset_splits}) achieves 3.7114/2.7614 (seen) and 5.1400/4.3770 (unseen), comparable to these published values and supporting the validity of the evaluation protocol. Despite using $7.7\times$ fewer neural parameters, YOLOv26-1D-Eff+RF matches the ResNet baseline on seen subjects, although it trails it on unseen subjects.

\subsection{Accuracy--Speed Balance}
Table~\ref{tab:final_balance} summarizes the relationship between trajectory accuracy, model footprint, and deployment latency for the strongest-performing models.

\begin{table}[htbp]
\caption{Final Accuracy--Latency Comparison (Seen Subjects; Pi Latency of the Base TFLite Backbone)}
\label{tab:final_balance}
\centering
\scriptsize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Neural Params} & \textbf{ATE} & \textbf{RTE} &
\textbf{Pi Latency} \\
 & & \textbf{(m)} & \textbf{(m)} & \textbf{(ms/window)} \\
\midrule
YOLOv26-1D-Eff & 598,530 & 4.1649 & 3.1488 & 25.31 \\
\textbf{YOLOv26-1D-Eff + RF} & \textbf{598,530} & \textbf{3.5848} & \textbf{2.6081} & \textbf{25.31} \\
MobileNetV2 + RF & 2,642,562 & 3.8009 & 2.4261 & 80.08 \\
RoNIN ResNet & 4,634,882 & 3.7114 & 2.7614 & 89.42 \\
\bottomrule
\end{tabular}
}
\end{table}
...
MobileNetV2+RF achieves the lowest RTE (2.4261~m), but its ATE (3.8009~m) is higher than that of YOLOv26-1D-Eff+RF (3.5848~m); it also requires 4.4$\times$ more neural parameters and exceeds the 50~ms update budget with a backbone latency of 80.08~ms. RoNIN-ResNet has the largest footprint and highest latency, while providing comparable trajectory accuracy on seen subjects. In contrast, YOLOv26-1D-Eff+RF achieves competitive ATE/RTE with a 0.60M-parameter backbone whose Raspberry Pi latency of 25.31~ms is about half the 50~ms update budget. These results highlight YOLOv26-1D-Eff+RF as a favorable operating point in the accuracy--latency--footprint tradeoff space for edge inertial navigation.
...
We presented a performance-aware inertial odometry framework that balances trajectory quality, model footprint, and inference cost for edge deployment. The proposed two-stage pipeline combines YOLOv26-1D-Eff, a lightweight 1D backbone whose attention and fusion modules are sized to the short token sequence it processes, with ResidualNav, a supervised Random Forest residual corrector that reduces local velocity error before trajectory integration. On the RoNIN benchmark, this design lowers Relative Trajectory Error with respect to the uncorrected backbone on both seen and unseen subjects and matches the RoNIN-ResNet baseline on seen subjects, while using 87.1\% fewer neural parameters. The residual-correction idea also transfers to NeurIT and RIDI, indicating that predictable local residuals can be exploited across different odometry pipelines. On a Raspberry Pi 3 B+, the YOLOv26-1D-Eff backbone runs in 25.31~ms per window, within the 50~ms update budget and $3.5\times$ faster than RoNIN-ResNet, while using $3.4\times$ less energy per run. Overall, these results suggest that edge inertial odometry should be evaluated as a complete system, where model size, runtime, and correction overhead are considered alongside trajectory error.
...
\bibitem{a2dio2022}
Y.~Wang, H.~Cheng and M.~Q.-H.~Meng,
``A2DIO: Attention-Driven Deep Inertial Odometry for Pedestrian
Localization Based on 6D IMU,''
in \textit{Proc.\ IEEE Int.\ Conf.\ Robotics and Automation (ICRA)},
2022, pp.~819--825.
...
% \bibitem{deepbias2022}
% D.\ Mueller \textit{et al.},
% ``Deep IMU Bias Inference for Robust Visual-Inertial Odometry
% with Factor Graphs,''
% \textit{arXiv preprint arXiv:2211.04517}, 2022.

\bibitem{deepbias2022}
R.~Buchanan, V.~Agrawal, M.~Camurri, F.~Dellaert, and M.~Fallon,
``Deep IMU Bias Inference for Robust Visual--Inertial Odometry
With Factor Graphs,''
\textit{IEEE Robotics and Automation Letters}, vol.~8, no.~1,
pp.~41--48, Jan.~2023.
