...
def get_model(arch):
    ...
    print("[ResNet1D] Plain baseline mode: training from scratch.")
...
def test_sequence(args):
    ...
    inference_times_per_sample = []  # milliseconds per sample
    sequence_times = {}  # milliseconds per sequence
    ...
    for data in test_data_list:
        ...
        seq_elapsed = (time.time() - seq_start) * 1000.0  # ms
        sequence_times[data] = seq_elapsed
        ...
        time_per_sample = seq_elapsed / num_samples if num_samples > 0 else 0.0
        inference_times_per_sample.extend([time_per_sample] * num_samples)
        ...
        print('Sequence {}, loss {} / {}, ate {:.6f}, rte {:.6f}, seq_time={:.2f}ms, time_per_sample={:.4f}ms'.format(
            data, losses, np.mean(losses), ate, rte, seq_elapsed, time_per_sample))
    ...
    print('\n' + '='*80)
    print('[TIMING SUMMARY] ResNet Baseline Plain')
    print(f"Mean time per sample: {np.mean(inference_times_per_sample):.4f} ms")
    print(f"Sequence times: {sequence_times}")
    print('='*80 + '\n')
