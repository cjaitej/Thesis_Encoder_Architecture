...
def get_model(args, **kwargs):
    ...
    elif args.type == 'tcn_transformer':
        network = TCNTransformerNetwork(
...
def test(args, **kwargs):
    ...
    for idx, data in enumerate(test_data_list):
        ...
        with torch.no_grad():
            ...
            if T <= window_size:
                preds = network(feat)
                preds = preds.squeeze(0).cpu().numpy()[:, :_output_channel]

            else:
                pred_sum = np.zeros((T, _output_channel))
                pred_count = np.zeros((T, 1))

                for start in range(0, T - window_size + 1, step_size):
                    end = start + window_size

                    pred = network(feat[:, start:end, :])
                    pred = pred.squeeze(0).cpu().numpy()

                    pred_sum[start:end] += pred[:, :_output_channel]
