...
def reset_bn_stats(model, new_momentum=0.1):
    """
    Reset BatchNorm1d running stats before domain transfer fine-tuning.
    """
    reset_count = 0
...
def _build_resnet(arch='resnet18'):
    if arch == 'resnet18':
        return ResNet1D(
            num_inputs=_input_channel,
...
def get_model(arch, load_pretrained=True, pretrain_ckpt='../output/pretrain_ridi_resnet_v2/checkpoints/checkpoint_latest.pt'):
    network = _build_resnet(arch)

    if load_pretrained:
        _pretrain_path = pretrain_ckpt
        _state = torch.load(_pretrain_path, map_location='cpu')
        _weights = _state['model_state_dict'] if 'model_state_dict' in _state else _state
        network.load_state_dict(_weights)
        print(f"[ResNet1D] Loaded RIDI pretrained weights")
        if 'epoch' in _state:
            print(f"[ResNet1D] Checkpoint epoch: {_state['epoch']}")
        ...
        print("[ResNet1D] Pretrained weights verified. BN stats reset. Ready.")
    else:
        print("[ResNet1D] Training from scratch (no pretrained checkpoint loaded).")

    print(f"[ResNet1D] Parameters: {network.get_num_params():,}")
    return network
...
def train(args, **kwargs):
    ...
    stats_path = get_stats_path(args.dataset)
    stats = np.load(stats_path, allow_pickle=True).item()
    norm_mean = torch.tensor(stats['mean'], dtype=torch.float32, device=device).view(1, 6, 1)
    norm_std = torch.tensor(stats['std'], dtype=torch.float32, device=device).view(1, 6, 1)
    ...
    network = get_model(
        args.arch,
        load_pretrained=(args.dataset == 'ronin' and not args.no_pretrain),
    ...
    init_train_targ, init_train_pred = run_test(
        network, train_loader, device, eval_mode=False, norm_mean=norm_mean, norm_std=norm_std
    )
    ...
    if val_loader is not None:
        init_val_targ, init_val_pred = run_test(
            network, val_loader, device, norm_mean=norm_mean, norm_std=norm_std
        )
...
def test_sequence(args):
    ...
    network = get_model(args.arch, load_pretrained=False)
    ...
    for data in test_data_list:
        ...
        targets, preds = run_test(network, seq_loader, device, True, norm_mean=norm_mean, norm_std=norm_std)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_pretrain', action='store_true',
                        help='Disable loading pretrained weights and train from scratch.')
    parser.add_argument('--pretrain_ckpt', type=str,
                        default='../output/pretrain_ridi_resnet_v2/checkpoints/checkpoint_latest.pt')
